...
<pre>
    git clone git://github.com/scrooloose/nerdtree.git
                ...
                <p class="para">
                Restart Vim and we can explore NERDTree. Enter <code>:NERDTree</code> in command line mode and NERDTree opens as a split. Arrow keys or Vim navigational keys <code>hjkl</code> could be used for moving around the hierarchy. <code>o</code> canbe used for expanding the folder or opening the file as new split depending on whether the cursor is on folder or a file. Open a file in a tab by moving the cursor to that file name and pressing the key <code>t</code>. Another useful key is <code>p</code> for navigating to the root folder, when the cursor is on child folder. You can use <code>go</code> for previewing the selected file instead of opening it for editing.
                </p>
    		</div>
                ...
                <table class="zebra-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                        	...
                        	<td>For inserting the characters not in the keyboard. For example CTRL-kCt inserts a cent(¢) character</td>
                        </tr>
                    </tbody>
                </table>
            </div>
